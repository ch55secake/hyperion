...
def parse_trading_results(content: str) -> Dict[str, Any]:
    ...
    dir_alpha_match = re.search(r'Directional Strategy:.*?Alpha vs Buy&Hold:\s*([-+]?[0-9]*\.?[0-9]+)%', content, re.DOTALL)

    if dir_value_match:
        result['trading_simulation']['strategies']['directional']['final_value'] = float(dir_value_match.group(1).replace(',', ''))
    if dir_return_match:
    ...
    adapt_value_match = re.search(r'Adaptive Threshold Strategy:.*?Final Value:\s*\$([0-9,]+\.[0-9]{2})', content, re.DOTALL)
    adapt_return_match = re.search(r'Adaptive Threshold Strategy:.*?Total Return:\s*([-+]?[0-9]*\.?[0-9]+)%', content, re.DOTALL)
    adapt_trades_match = re.search(r'Adaptive Threshold Strategy:.*?Number of Trades:\s*(\d+)', content, re.DOTALL)
    adapt_alpha_match = re.search(r'Adaptive Threshold Strategy:.*?Alpha vs Buy&Hold:\s*([-+]?[0-9]*\.?[0-9]+)%', content, re.DOTALL)

    if adapt_value_match:
        result['trading_simulation']['strategies']['adaptive_threshold']['final_value'] = float(adapt_value_match.group(1).replace(',', ''))
    if adapt_return_match:
        result['trading_simulation']['strategies']['adaptive_threshold']['total_return'] = float(adapt_return_match.group(1))
    if adapt_trades_match:
        result['trading_simulation']['strategies']['adaptive_threshold']['number_of_trades'] = int(adapt_trades_match.group(1))
    if adapt_alpha_match:
        result['trading_simulation']['strategies']['adaptive_threshold']['alpha_vs_buy_hold'] = float(adapt_alpha_match.group(1))
    ...
    hold_alpha_match = re.search(r'Hold Days Strategy:.*?Alpha vs Buy&Hold:\s*([-+]?[0-9]*\.?[0-9]+)%', content, re.DOTALL)

    if hold_value_match:
        result['trading_simulation']['strategies']['hold_days']['final_value'] = float(hold_value_match.group(1).replace(',', ''))
